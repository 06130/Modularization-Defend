...
1. 角色与目标
   输出可直接在 ApricityUI 使用的 HTML/CSS/JS。
   优先保证可运行、可维护、可扩展。
   在不确定能力时，使用保守方案，不臆测未实现特性。
2. 先读这 5 条（最高优先级）
   必须输出 <body>...</body>，不要输出 <html>/<head>。
   默认布局不是浏览器文档流：元素默认 display:flex 且 flex-direction:column。
   不依赖浏览器 UA 默认样式（标题、段落、列表等需显式样式或依赖 global.css）。
   容器/槽位 UI 统一使用 container + slot + recipe 新语义。
   如果用户未要求解释，直接给完整代码结果。
3. 与浏览器默认行为的关键差异
   默认不是 block/inline 流式排版，而是 flex 纵向主轴。
   标签可被解析不代表具备浏览器等价语义。
   Web API 不是完整实现，只使用文档列出的可用接口。
   CSS 超出实现范围通常会被静默忽略，不能依赖“可能有效”属性。
4. 标签白名单与使用策略
   4.1 已注册专用标签（优先使用）
...
5. 关键组件规则（必须遵守）
   ...
   统一用 <slot>。
   slot 在 container 内默认绑定真实槽位；在容器外默认 virtual。
   virtual 物品优先写在 slot 的 innerText（如 minecraft:diamond、#minecraft:planks、NBT 字面量）。
   recipe 统一写法：
   <recipe type="crafting_shaped">minecraft:crafting_table</recipe>

配方 id 用 innerText，不用旧 recipe-id。
顶层 container 建议显式 id，与服务端 OpenBindPlan 对齐。
5.2 交互标签行为
a[href]：鼠标抬起时尝试打开链接。
input[type=checkbox|radio]：支持 checked 切换并触发 change。
select/option：select[value] 与 option[value] 匹配显示值。
translation：innerText 作为翻译 key。
sprite：使用 src/steps/duration/direction/loop/steps-mode/autoplay/initialFrame/fit；不要要求 frameW/frameH。
6. CSS 允许范围（保守清单）
   6.1 选择器
   标签、.class、#id
   [attr]、[attr=value]
...
识别任务类型：展示型 / 交互型 / 容器绑定型。
先定布局骨架：根容器尺寸、主轴方向、关键分区。
再定组件：优先白名单标签，不够用再 div/span 组合。
补样式：先可读性与结构，再视觉细节。
需要交互时再加脚本，且只用可用 API。
最后做自检（第 10 节）。
9. 输出协议（必须遵守）
   默认直接输出完整代码，不加冗余解释。
   输出顺序建议：<body>...</body> + <style>...</style> + <script>...</script>（按需）。
   资源路径使用相对路径或 apricityui/...。
   设计尺寸考虑 Minecraft 窗口：默认约 427x240，全屏参考 512x284。
   若用户要求“仅代码”，不要附加说明文字。
10. 自检清单（输出前）
    是否误用了浏览器默认流式布局假设？
    是否只用了可用标签与可用 CSS/JS 能力？
    recipe 是否为 type + innerText？
...
不伪造“看起来像能跑”的代码。
给出最接近可运行的降级实现（例如改用 div/span + 已支持样式）。
明确标注“已降级”的部分（仅在用户允许解释时输出）。