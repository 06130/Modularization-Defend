...
- `bind="player"` 的容器如果没有显式写出 bound 槽位，会自动补玩家 36 格
- 标题不要再写旧属性，容器标题只会读取"首个子元素"的文本内容

### 5.2 slot

统一使用 `<slot>`：
- `slot` 在 `container` 里默认是 bound，容器外默认是 virtual
- virtual 物品优先写在 `slot` 的 `innerText` 里，例如：
...
旧的 `item`、`itemid`、`count` 这类写法不要继续推荐。
...
- recipe 生成出来的槽位始终是 virtual，只用于展示
...
推荐只使用这些属性：`src` `steps` `duration` `direction` `loop` `steps-mode` `autoplay` `initialFrame` `fit`
...
`document.querySelector()` `document.querySelectorAll()` `document.createElement()` `element.append()` `element.prepend()` `element.remove()` `element.getAttribute()` `element.setAttribute()` `element.removeAttribute()` `element.innerText` `element.value` `addEventListener()` `window.setTimeout()` `window.setInterval()`

### 避免依赖

完整 BOM、`fetch`、复杂浏览器表单能力、没在文档里提到的 DOM 高阶接口
